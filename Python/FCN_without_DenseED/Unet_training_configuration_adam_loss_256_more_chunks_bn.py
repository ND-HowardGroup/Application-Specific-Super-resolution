...
from model.unet import UnetN2Nv2
import random
import pandas as pd
...
test_type = 1
CSV_path = '/afs/crc.nd.edu/user/v/vmannam/Desktop/Spring21/Apr21/1104/SR_training_old/'
...
model = UnetN2Nv2(in_channels, out_channels).to(device) #set the model and assign to the device too
...
print('SR Unet BN Model summary >>>>>>>> ',model) #diasblaed model print
...
ckpt_dir = '/afs/crc.nd.edu/user/v/vmannam/Desktop/Spring21/Apr21/1104/SR_training_old/'
#train function
...
def train(epochs):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (noisy_input, clean_target) in enumerate(dataloader_train):
            ...
            for i in range(batch_size):
                ...
                denoised_full[i, 0, 0:part_size, mid_size1:mid_size2] = denoised[(i*9)+1,:,:,:]
                denoised_full[i, 0, 0:part_size, part_size:full_size] = denoised[(i*9)+2,:,:,:]
                denoised_full[i, 0, mid_size1:mid_size2, 0:part_size] = denoised[(i*9)+3,:,:,:]
                denoised_full[i, 0, mid_size1:mid_size2, mid_size1:mid_size2] = denoised[(i*9)+4,:,:,:]
                denoised_full[i, 0, mid_size1:mid_size2, part_size:full_size] = denoised[(i*9)+5,:,:,:]
                denoised_full[i, 0, part_size:full_size, 0:part_size] = denoised[(i*9)+6,:,:,:]
                denoised_full[i, 0, part_size:full_size, mid_size1:mid_size2] = denoised[(i*9)+7,:,:,:]
                denoised_full[i, 0, part_size:full_size,part_size:full_size] = denoised[(i*9)+8,:,:,:]
                
                clean_full[i, 0, 0:part_size, 0:part_size] = clean_target[i*9,:,:,:] 
                clean_full[i, 0, 0:part_size, mid_size1:mid_size2] = clean_target[(i*9)+1,:,:,:]
                clean_full[i, 0, 0:part_size, part_size:full_size] = clean_target[(i*9)+2,:,:,:]
                clean_full[i, 0, mid_size1:mid_size2, 0:part_size] = clean_target[(i*9)+3,:,:,:]
                clean_full[i, 0, mid_size1:mid_size2, mid_size1:mid_size2] = clean_target[(i*9)+4,:,:,:]
                clean_full[i, 0, mid_size1:mid_size2, part_size:full_size] = clean_target[(i*9)+5,:,:,:]
                clean_full[i, 0, part_size:full_size, 0:part_size] = clean_target[(i*9)+6,:,:,:]
                clean_full[i, 0, part_size:full_size, mid_size1:mid_size2] = clean_target[(i*9)+7,:,:,:]
                clean_full[i, 0, part_size:full_size,part_size:full_size] = clean_target[(i*9)+8,:,:,:]
                
                
            loss = F.mse_loss(denoised_full, clean_full, reduction='sum') #this is the error bwtween images of 256x256 MSE loss
            loss.backward()
        ...
        rmse = np.sqrt(mse / (n_train_pixels*train_len))
        scheduler.step(rmse)
        ...
        with torch.no_grad():
            ...
            for batch_idx, (noisy, clean) in enumerate(dataloader_test):
                ...
                if epoch % 50 == 0:
                    ...
                    io.savemat('predict_super_resolution_%d.mat'%epoch, dict([('predict_super_resolution1_test',np.array(par_babu))])) 
    ...
    np.savetxt(path+'/Unet_BN_train_RMSE_1104_2021_'+timestr_plt+'.txt', np.array(mse_loss_train))
    np.savetxt(path+'/Unet_BN_test_RMSE_1104_2021_'+timestr_plt+'.txt', np.array(mse_loss_test))
    # save model
    torch.save(model.state_dict(), ckpt_dir + "/Unet_BN_model_epoch{}.pth".format(epoch))
    torch.save(model,ckpt_dir+'Unet_BN_model_SR_1104_2021_'+timestr_plt+'.pt')
    #plots 
    ...
    plt.savefig(path+'/Unet_RMSE_loss_1104_2021_'+timestr_plt+'.png')

if __name__ == "__main__":
    #Trainer(default_save_path=’/your/path/to/save/checkpoints’)
    path = '/afs/crc.nd.edu/user/v/vmannam/Desktop/Spring21/Apr21/1104/SR_training_old/'
