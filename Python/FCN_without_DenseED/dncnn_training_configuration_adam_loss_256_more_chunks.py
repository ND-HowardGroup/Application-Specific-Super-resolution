...
#model = UnetN2N(in_channels, out_channels).to(device) #set the model and assign to the device too
model = DnCNN(depth=17, n_channels=64, image_channels=1, use_bnorm=True, kernel_size=3).to(device) #DnCNN model
...
print('SR dncnn Model summary >>>>>>>> ',model) #diasblaed model print
...
def train(epochs):
    ...
    np.savetxt(path+'/dncnn_train_RMSE_1104_2021_'+timestr_plt+'.txt', np.array(mse_loss_train))
    np.savetxt(path+'/dncnn_test_RMSE_1104_2021_'+timestr_plt+'.txt', np.array(mse_loss_test))
    # save model
    torch.save(model.state_dict(), ckpt_dir + "/dncnn_model_epoch{}.pth".format(epoch))
    torch.save(model,ckpt_dir+'dncnn_model_SR_1104_2021_'+timestr_plt+'.pt')
    #plots 
    ...
    plt.savefig(path+'/dncnn_RMSE_loss_1104_2021_'+timestr_plt+'.png')
