...
from model.unet import UnetN2N
import random
import pandas as pd
...
print('SR Unet Model summary >>>>>>>> ',model) #diasblaed model print
...
def train(epochs):
    ...
    np.savetxt(path+'/Unet_train_RMSE_1104_2021_'+timestr_plt+'.txt', np.array(mse_loss_train))
    np.savetxt(path+'/Unet_test_RMSE_1104_2021_'+timestr_plt+'.txt', np.array(mse_loss_test))
    # save model
    torch.save(model.state_dict(), ckpt_dir + "/Unet_model_epoch{}.pth".format(epoch))
    torch.save(model,ckpt_dir+'Unet_model_SR_1104_2021_'+timestr_plt+'.pt')
    #plots 
