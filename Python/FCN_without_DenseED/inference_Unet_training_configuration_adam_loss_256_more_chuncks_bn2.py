...
class DataGenerator(data.Dataset):
    def __init__(self, num_exps=100, batch_size=1, dim=(256,256), n_channels=1, shuffle=True, train = True, validation = False, test = False, test_type=1, transform=None):
        'Initialization'
        self.dim = dim
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        # Initialization
        X = np.empty(((self.batch_size)*1, self.n_channels, *self.dim)) #total  batch size here includes slices
        Y = np.empty(((self.batch_size)*1, self.n_channels, *self.dim))
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            X[i*1:(i+1)*1, 0, ...] = imx1 / 65535 - 0.5
            #print('XXXXXX ',X.shape)
            Y[i*1:(i+1)*1, 0, ...] = lbx1 / 65535 - 0.5    #.squeeze()
...
ckpt_dir = '/afs/crc.nd.edu/user/v/vmannam/Desktop/Spring21/Apr21/1104/SR_training_old/Unet_BN_model_SR_1104_2021_20210411_152852.pt'
...
#             noisy_full[i, 0, part_size:full_size,part_size:full_size] = noisy[(i*9)+8,:,:,:]
            noisy_full[i, 0, 0:full_size, 0:full_size] = noisy[i*1,:,:,:] 
            denoised_full[i, 0, 0:full_size, 0:full_size] = denoised[i*1,:,:,:] 
...
#             denoised_full[i, 0, part_size-boundary:part_size+boundary, part_size:full_size] = test4[part_size-mid_size1-boundary:part_size-mid_size1+boundary,:]
            ...
            clean_full[i, 0, 0:full_size, 0:full_size] = clean[i*1,:,:,:] 
...
#             clean_full[i, 0, part_size:full_size,part_size:full_size] = clean[(i*9)+8,:,:,:]
            ...
            noisy_full = (noisy_full+0.5)*65535
            clean_full = (clean_full+0.5)*65535
            denoised_full = (denoised_full+0.5)*65535
            
            noisy_full =  np.clip(noisy_full,0,65535)
            clean_full = np.clip(clean_full,0,65535)
            denoised_full = np.clip(denoised_full,0,65535)
            
            import imageio
            imageio.imwrite('unet_bn_DL_image2.tif', noisy_full)
            imageio.imwrite('unet_bn_target_SR_image2.tif', clean_full)
            imageio.imwrite('unet_bn_Est_SR_image2.tif', denoised_full)
