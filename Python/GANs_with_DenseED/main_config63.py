...
model_g = DenseED(in_channels=1, out_channels=1, 
                blocks=[3, 6, 3],
                growth_rate=16, 
                init_features=48,
                drop_rate=0.0,
                bn_size=8,
                bottleneck='store_true',
...
def train(epochs):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (noisy_input, clean_target) in enumerate(dataloader_train):
            ...
            mse_gloss = F.l1_loss(imgs_fake, clean_target, reduction='sum')
            with torch.no_grad():
                ssim_val = cal_ssim(clean_target, imgs_fake)#.sum().item() #sum of all images in the batch
                #print('>>>>>>>>',ssim_val)
                ssim_val = ssim_val.sum()

            new_loss = 0.7*mse_gloss+0.1*g_loss+0.2*ssim_val
            ...
            loss = F.l1_loss(denoised_full, clean_full, reduction='sum') #this is the error bwtween images of 256x256 MSE loss replaced with reduction=sum from size_average= False
            # loss.backward()
        ...
        with torch.no_grad():
            ...
            for batch_idx, (noisy, clean) in enumerate(dataloader_test):
                ...
                loss = F.l1_loss(denoised, clean, reduction='sum') #replaced with reduction=sum from size_average= False
                mse_test += loss.item()
...
if __name__ == "__main__":
    ...
    path = '/afs/crc.nd.edu/user/g/ganantha/Test/Balayya/SR_training/Model2_DenseED_GANs/src_base/config63/' #change the folder name every time
