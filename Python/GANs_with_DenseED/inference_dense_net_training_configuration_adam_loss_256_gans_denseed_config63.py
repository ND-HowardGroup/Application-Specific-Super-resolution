...
from load_dataset_new import load_dataset 
...
ckpt_dir = '/afs/crc.nd.edu/user/g/ganantha/Test/Balayya/SR_training/Model2_DenseED_GANs/src_base/config63/GANs_model_SR_20210317_224844.pt'

model_g = torch.load(ckpt_dir)
model_g.eval()
print('SR DenseED Model summary >>>>>>>> ',model_g) #diasblaed model print
...
scheduler = ReduceLROnPlateau(optimizer_G, mode='min', factor=0.1, patience=10,
...
def cal_ssim(clean, noisy, normalized=True):
    ...
    ssim1 = np.array([ssim(clean[i, 0], noisy[i, 0], data_range=65535) 
...
def train(epochs):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (noisy, clean) in enumerate(dataloader_test):
            ...
            import imageio
            imageio.imwrite('DL_image.png', noisy_full)
            imageio.imwrite('target_SR_image.png', clean_full)
            imageio.imwrite('Est_SR_image.png', denoised_full)
            
            
            imageio.imwrite('DL_image.tif', noisy_full)
            imageio.imwrite('target_SR_image.tif', clean_full)
            imageio.imwrite('Est_SR_image.tif', denoised_full)
...
if __name__ == "__main__":
    ...
    path = '/afs/crc.nd.edu/user/g/ganantha/Test/Balayya/SR_training/Model2_DenseED_GANs/src_base/config63/'
    ...
    train(Epochs)
    t2 = time.time()
    #torch.save(model_H,'ConvNet2211_Hadamard_03.pt')
